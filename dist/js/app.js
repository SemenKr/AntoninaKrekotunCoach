(() => {
    ...
    function loopFix(_temp) {
        ...
        const gridEnabled = swiper.grid && params.grid && params.grid.rows > 1;
        if (slides.length < slidesPerView + loopedSlides) showWarning("Swiper Loop Warning: The number of slides is not enough for loop mode, it will be disabled and not function properly. You need to add more slides (or make duplicates) or lower the values of slidesPerView and slidesPerGroup parameters"); else if (gridEnabled && "row" === params.grid.fill) showWarning("Swiper Loop Warning: Loop mode is not compatible with grid.fill = `row`");
        const prependSlidesIndexes = [];
        const appendSlidesIndexes = [];
        let activeIndex = swiper.activeIndex;
        if ("undefined" === typeof activeSlideIndex) activeSlideIndex = swiper.getSlideIndex(slides.filter((el => el.classList.contains(params.slideActiveClass)))[0]); else activeIndex = activeSlideIndex;
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) {
            const thumbsSwiper = new swiper_core_Swiper(".training__slider-thumbs", {
            ...
            new swiper_core_Swiper(".training__slider", {
            ...
            new swiper_core_Swiper(".feedback__slider", {
    ...
    menuToggle.addEventListener("click", (() => {
        const isOpened = "true" === menuToggle.getAttribute("aria-expanded");
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        function formValidate(form) {
            ...
            for (let index = 0; index < formReq.length; index++) {
                ...
                if (input.classList.contains("_email")) if (emailTest(input)) {
                    formAddError(input);
                    error++;
